...
export const validate = <Data>(schema: ZodSchema, data: Data): ValidateReturnType<Data> => {
  ...
  try {
    ...
    if (flattenedError.formErrors.length) {
      ...
      Object.keys(flattenedError.fieldErrors).forEach((key) => {
        setupError[key] =
          Array.isArray(flattenedError.fieldErrors[key]) && flattenedError.fieldErrors[key].length
